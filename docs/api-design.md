# 接口设计文档（前后端对接）

## 1. 基础信息

- 后端基地址：`http://localhost:8080`
- API 前缀：`/api`
- 用户角色：`USER`（普通用户）、`ADMIN`（管理员）
- 认证机制：双令牌
- `Authorization: Bearer <accessToken>`
- `X-Refresh-Token: <refreshToken>`

## 2. 统一响应结构

```json
{
  "code": 200,
  "message": "success",
  "data": {}
}
```

- `code`：业务码（`200/400/401/403/404/500`）
- `message`：提示信息
- `data`：业务数据，失败时通常为 `null`

注意：当前项目统一返回 HTTP 200，前端请以 `code === 200` 判定成功。

## 3. 鉴权分级

### 3.1 免鉴权接口

- `GET /api/health`
- `POST /api/users/register`
- `POST /api/auth/login`

### 3.2 登录鉴权接口（双令牌）

- `POST /api/auth/logout`
- `GET /api/users`
- `GET /api/users/{id}`

### 3.3 管理员接口（双令牌 + ADMIN）

- `POST /api/admin/users`

## 4. 数据模型

### 4.1 User

| 字段 | 类型 | 说明 |
| --- | --- | --- |
| id | number | 用户 ID |
| username | string | 用户名 |
| email | string | 邮箱 |
| role | string | `USER` 或 `ADMIN` |

### 4.2 RegisterRequest

| 字段 | 类型 | 必填 | 约束 |
| --- | --- | --- | --- |
| username | string | 是 | 非空，长度 <= 50 |
| email | string | 是 | 非空，合法邮箱 |
| password | string | 是 | 非空，长度 6-64 |

### 4.3 LoginRequest

| 字段 | 类型 | 必填 | 约束 |
| --- | --- | --- | --- |
| username | string | 是 | 非空 |
| password | string | 是 | 非空 |

### 4.4 LoginResponse

| 字段 | 类型 | 说明 |
| --- | --- | --- |
| userId | number | 用户 ID |
| username | string | 用户名 |
| email | string | 邮箱 |
| role | string | `USER` 或 `ADMIN` |
| accessToken | string | 访问令牌 |
| refreshToken | string | 刷新令牌 |

### 4.5 AdminCreateUserRequest

| 字段 | 类型 | 必填 | 约束 |
| --- | --- | --- | --- |
| username | string | 是 | 非空，长度 <= 50 |
| email | string | 是 | 非空，合法邮箱 |
| password | string | 是 | 非空，长度 6-64 |
| role | string | 是 | `USER` 或 `ADMIN` |

## 5. 接口详情

### 5.1 健康检查

- `GET /api/health`

成功示例：

```json
{
  "code": 200,
  "message": "success",
  "data": {
    "status": "UP",
    "time": "2026-04-24T14:10:00.000"
  }
}
```

### 5.2 用户注册（默认 USER）

- `POST /api/users/register`
- `Content-Type: application/json`

请求：

```json
{
  "username": "alice",
  "email": "alice@example.com",
  "password": "123456"
}
```

成功：

```json
{
  "code": 200,
  "message": "success",
  "data": {
    "id": 2,
    "username": "alice",
    "email": "alice@example.com",
    "role": "USER"
  }
}
```

### 5.3 登录（获取双令牌）

- `POST /api/auth/login`

请求：

```json
{
  "username": "alice",
  "password": "123456"
}
```

成功：

```json
{
  "code": 200,
  "message": "success",
  "data": {
    "userId": 2,
    "username": "alice",
    "email": "alice@example.com",
    "role": "USER",
    "accessToken": "xxx",
    "refreshToken": "yyy"
  }
}
```

### 5.4 退出登录（双令牌）

- `POST /api/auth/logout`
- 请求头必须包含：
- `Authorization: Bearer <accessToken>`
- `X-Refresh-Token: <refreshToken>`

成功：

```json
{
  "code": 200,
  "message": "success",
  "data": null
}
```

### 5.5 查询用户列表（双令牌）

- `GET /api/users`
- 需要双令牌请求头

成功：

```json
{
  "code": 200,
  "message": "success",
  "data": [
    {
      "id": 1,
      "username": "admin",
      "email": "admin@example.com",
      "role": "ADMIN"
    },
    {
      "id": 2,
      "username": "alice",
      "email": "alice@example.com",
      "role": "USER"
    }
  ]
}
```

### 5.6 查询用户详情（双令牌）

- `GET /api/users/{id}`
- 需要双令牌请求头

### 5.7 管理员创建用户（双令牌 + ADMIN）

- `POST /api/admin/users`
- 需要双令牌请求头

请求：

```json
{
  "username": "bob",
  "email": "bob@example.com",
  "password": "123456",
  "role": "ADMIN"
}
```

## 6. 业务错误码

- `400`：参数错误、用户名重复
- `401`：令牌缺失/无效/过期，或登录凭证错误
- `403`：非管理员访问管理员接口
- `404`：资源不存在
- `500`：服务端异常

## 7. 前端对接要点

- 登录成功后保存 `accessToken + refreshToken`。
- 访问受保护接口时同时带两个请求头。
- 任意接口返回 `code !== 200` 时统一提示 `message`。
